...
\begin{codebox}
  \Procname{IsingDist}
  \zi  Comment Plot magnetization versus temperature for the Ising model.
  \zi  Comment WARNING: Use only for $N=2,3,4$.
  \li  Require:  Affinity constant $J$
  \li    Require:  Size (side length) of grid $N$
  \li    Require:  Number of normalized temperatures, and list of
  \li    temperatures around $kT_c/J = 2.269$ when $J=1$
...
\begin{minipage}[t]{1.0\linewidth}
\begin{codebox}
  \Procname{IsingMetropolis}
  \zi  Comment Find magnetization for a temperature for the Ising model
  \zi    using Metropolis algorithm
  \li  Require: Affinity constant $J$, normalized temperature
  \li  Require: Size of grid $N$, number of steps for MCMC algorithm
...
\begin{codebox}
  \Procname{GibbsImage}
  \zi  Comment Use Gibbs sampling to restore a blurred picture
  \zi    Require: Size $N$ of $N \times N$ grid, number of iterations of
    raster scan.
  ...
  \li      straying too far from the data \( \omega^{\text{blurred}} \) while the
  \li      second term represents the desire to align neighboring pixel values
  \li      making them conform to the prior notion of a generic image.
  ...
  \zi Comment: Uses utility functions \proc{makeNoise}, \proc{roundPicture}
  \li\proc{roundPicture}($M$)
  \li Round real number to nearest of $0$, $1$.
  \li\proc{makeNoise}
  \li Generate some random noise (Gaussian, mean $0$, standard deviation
